...
export async function searchTrucks(qRaw: string): Promise<SearchResult[]> {
  ...
  const [insp, intk, qts] = await Promise.all([
    db.execute(sql`
      SELECT id, qc_number, vin, stock, data->>'vehicle' AS vehicle, status, archived,
             EXTRACT(EPOCH FROM updated_at) * 1000 AS updated_ms
      FROM inspections
      WHERE UPPER(vin) LIKE ${like} OR UPPER(stock) LIKE ${like} OR UPPER(qc_number) LIKE ${like}
  ...
  for (const r of rowsOf(insp)) {
    out.push({
      ...
      archived: !!r.archived,
      committed: true, // an inspection row is a committed QC record by nature
      quoteId: null,
      intakeId: null,
