...
- Aug 2026, user-directed: the quote-screen "Commit quote" button was REMOVED — intake SAVE is the only sign-off; quotes normally stay uncommitted (photos/lines stay editable). Server commit-quote endpoint + committed guards kept for legacy/imported committed quotes. Photo upsert has an ownership guard (existing photo id can't be hijacked by another quoteId). Lightbox has ROTATE (client canvas, 1600px cap) for fixing mis-oriented legacy shots.
- User-approved photo policy (Aug 2026, supersedes earlier additive-only rule): while a quote is UNCOMMITTED, its photos are fully editable — saved intakes can ADD new photos ("extra" mode, xtra_ slots) and any photo can be DELETED (lightbox delete → DELETE /api/quoter/photos, server 409s once committed). Committed quotes stay fully frozen. Extras display merged into the walk-around set (only dmg_ shown apart); the /24 checklist counts guided slots only. Camera preloads server photos and has an upload retry queue.
