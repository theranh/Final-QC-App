...
- User-approved exception (Aug 2026): a SAVED (committed) intake may still ADD walk-around photos into EMPTY slots while its quote is uncommitted (camera addOnly mode; existing photos never replaceable). Committed quotes stay fully frozen. Camera also preloads server photos (resume) and has an upload retry queue for weak signal.
