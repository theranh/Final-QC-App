...
**Photos-only quote ids:** an intake's linked quoteId may have no quote row until the first quote save — treat "not found in sync" as a fresh quote (never an error), and hydrate committedBy before choosing a step so signed quotes stay read-only.
